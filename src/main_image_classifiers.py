...
# python main_image_classifiers.py --batch_size=128 --dataset_dir="/cluster/tufts/hugheslab/eharve06/CIFAR-10" --experiments_dir="/cluster/tufts/hugheslab/diag_VI" --kappa=23528.522 --lr_0=0.01 --model_arch="ResNet-50" --n=1000 --num_workers=0 --prior_dir="/cluster/tufts/hugheslab/eharve06/resnet50_torchvision" --prior_type="resnet50_torchvision" --random_state=1001 --tune
if __name__=="__main__":
    ...
    backbone_prior = priors.IsotropicGaussianPrior(prior_params=backbone_prior_params, use_tau_star=True)
    classifier_prior = priors.IsotropicGaussianPrior(num_params=classifier_num_params, use_tau_star=True)
    ...
    for epoch in range(epochs):
        ...
        if epoch == 0:
            columns = ["epoch", *[f"train_{key}" for key in train_metrics.keys()], "train_sec/epoch", *[f"val_or_test_{key}" for key in val_or_test_metrics.keys()]]
        ...
        row = [epoch, *[value for key, value in train_metrics.items()], train_epoch_end_time - train_epoch_start_time, *[value for key, value in val_or_test_metrics.items()]]
